...
public function Header() {
    ...
    // Title
    $this->Cell(0, 15, 'Infome de Cursos por Tipo', 0, false, 'C', 0, '', 0, false, 'M', 'M');
}
	...
	public function ColoredTable($header,$data) {
		...
		$fill = 0;
		foreach ($data as $row ){
			$this->Cell($w[0], 6, $row['titcurso'], 'LR', 0, 'L', $fill);
			$this->Cell($w[1], 6, $row['descripcurso'], 'LR', 0, 'L', $fill);
			$this->Cell($w[2], 6, number_format($row['costo']), 'LR', 0, 'R', $fill);
			$this->Cell($w[3], 6, $row['nomtipo'], 'LR', 0, 'L', $fill);
...
// column titles
$header = array('Titulo del Curso', 'Descripción del Curso', 'Costo', 'Tipo Curso');
