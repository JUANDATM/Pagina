...
try{
        $Cn = new PDO('pgsql:host=localhost;port=5432;dbname=Proyectox;user=postgres;password=1234');
...
function insertaAlumno($post){//arreglo que va a recbir es el $post
    $no = $post['nocontrol'];
    $carr = $post['carrera'];
    $nom = $post['nombre'];
    $tel= $post['telefono'];
    $sentencia = 'INSERT INTO "Escuela".alumno(nocontrol,carrera,nombre,telefono)' . 
                 "values('$no','$carr','$nom','$tel')";//no es numerico asi que no retorna nada
    return Ejecuta($sentencia);
    //die ($sentencia);
}
...
function consultaCurso($idcur){
    $query = 'SELECT idcurso,titcurso,descripcurso,costo,imagen,idtipo FROM "Escuela"."curso" WHERE ' . " idcurso = '$idcur' ORDER BY titcurso";
    return Consulta($query);
}
